...
仓库内默认保留了一份英文课程销售场景的示例数据，用于帮助中英文用户和开源评审快速跑通流程。请把这些数据视为 demo 内容，真实使用前需要替换为你自己的产品资料、价格、FAQ、销售规则和合规说明。
...
- 后台操作日志，记录登录、数据导出/备份、知识库、素材、客户、证书和配置等关键变更。
- 证书记录导入和查询 demo 流程。
...
`data/db.json` 包含英文课程销售场景的示例数据。它的作用是演示销售资料如何变成知识库，不是生产数据。
